...
<div class="panel panel-default" style="margin: 0 auto;width: 95%;">
	...
	<div class="panel-body">
	   	   <!--列表开始-->
	    <div class="row ${pageinfo.total<=0?"div1":"" }">
	    	<c:if test="${pageinfo.total<=0 }">没有数据！！！</c:if>
			...
			<nav aria-label="..." class="text-center">
			  <ul class="pagination">
			  	...
			  	<c:if test="${pageinfo.pageNum>1 }">
			  	 	<li><a href="${pageContext.request.contextPath }/goodsservlet?method=getGoodsListByTypeId&pageNum=${pageinfo.pageNum-1}&pageSize=${pageinfo.pageSize}&typeId=${typeId}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
			  	</c:if>
			   
			    <c:forEach var="pn" begin="1" end="${pageinfo.pages }" step="1">
			    ...
			    <c:if test="${pageinfo.pageNum==pageinfo.pages }">
			     <li class="disabled"><span aria-hidden="true">»</span></li>
			      ...
			      <c:if test="${pageinfo.pageNum<pageinfo.pages }">
			     <li><a href="${pageContext.request.contextPath }/goodsservlet?method=getGoodsListByTypeId&pageNum=${pageinfo.pageNum+1}&pageSize=${pageinfo.pageSize}&typeId=${typeId}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
			    </c:if>
		  </ul>
		</nav>
		</c:if>
